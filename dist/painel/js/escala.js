...
<div class="toast" id="toast">
  ...
  var inlineJs = [
    ...
    '',
    'function totalVagas() {',
    '  return escala.reduce((s, d) => s + d.turnos.reduce((ss, t) => ss + t.vagas, 0), 0);',
    '}',
    '',
    ...
    '    const headerClass  = dia.tipo === "intenso" ? "intenso" : "";',
    '    const circuloClass = dia.tipo === "intenso" ? "intenso" : dia.id === "seg" ? "padrao-seg" : "";',
    '    const tagClass  = dia.tipo === "intenso" ? "intenso" : "";',
    '    const tagLabel  = dia.tipo === "intenso" ? "🔥 Reforço" : "Padrão";',
    '    const subText   = dia.tipo === "intenso" ? "vagas — noite movimentada" : "vagas no total";',
    '    html += \'<div class="dia-bloco">\';',
    '    html += \'<div class="dia-header \' + headerClass + \'">\';',
    '    html += \'<div class="dia-info">\';',
    '    html += \'<div class="dia-circulo \' + circuloClass + \'">\';',
    '    html += \'<div class="dia-circulo-nome">\' + dia.sigla + \'</div>\';',
    '    html += \'<div class="dia-circulo-data">\' + dia.data + \'</div>\';',
    '    html += \'</div><div>\';',
    '    html += \'<div class="dia-nome-full">\' + dia.nome + \'</div>\';',
    '    html += \'<div class="dia-nome-sub">\' + totalDia + \' \' + subText + \'</div>\';',
    '    html += \'</div></div>\';',
    '    html += \'<div class="dia-tag \' + tagClass + \'">\' + tagLabel + \'</div>\';',
    '    html += \'</div>\';',
    ...
    '      html += \'<div class="turno-horario">\';',
    '      html += \'<span class="turno-icon">\' + turno.icon + \'</span>\';',
    '      html += \'<span class="turno-titulo">\' + turno.titulo + \'</span>\';',
    '      html += \'</div>\';',
    '      html += \'<span class="turno-vagas \' + (completo ? "completo" : "") + \'">\';',
    '      html += preenchidas + "/" + turno.vagas + " " + (completo ? "✅" : "vagas") + \'</span>\';',
    '      html += \'</div>\';',
    '      for (let i = 0; i < turno.vagas; i++) {',
    '        const valor = dados[i] || "";',
    '        const esc = valor.replace(/"/g, "&quot;");',
    '        html += \'<div class="vaga">\';',
    '        html += \'<div class="vaga-num">\' + (i + 1) + \'</div>\';',
    '        html += \'<div class="vaga-input-wrap">\';',
    '        html += \'<input type="text" class="vaga-input \' + (valor ? "preenchida" : "") + \'"\';',
    '        html += \' placeholder="Toque e escreva seu nome..." value="\' + esc + \'"\';',
    '        html += \' data-turno="\' + turno.id + \'" data-vaga="\' + i + \'" />\';',
    '        html += \'</div>\';',
    '        html += \'<button class="vaga-btn-remover \' + (valor ? "" : "hidden") + \'"\';',
    '        html += \' data-remover-turno="\' + turno.id + \'" data-remover-vaga="\' + i + \'">✕</button>\';',
    '        html += \'</div>\';',
    '      }',
    '      html += \'</div>\';',
    '    });',
    ...
    '      remover(btn.dataset.removerTurno, parseInt(btn.dataset.removerVaga));',
    '    });',
    '  });',
    '}',
    '',
    ...
    '  input.classList.toggle("preenchida", !!valor);',
    '  btn.classList.toggle("hidden", !valor);',
    '}',
    '',
    'async function salvar(input) {',
    '  const turnoId = input.dataset.turno;',
    '  const vagaIdx = parseInt(input.dataset.vaga);',
    '  const valor   = input.value.trim();',
    ...
    '  } catch (err) {',
    '    mostrarToast("⚠️", "Erro ao salvar", "erro");',
    '    console.error(err);',
    '  }',
    '}',
    '',
    ...
    '  try {',
    '    await db.ref("escalas/" + SEMANA_KEY + "/" + turnoId + "/" + vagaIdx).set(null);',
    '  } catch (err) {',
    '    mostrarToast("⚠️", "Erro", "erro");',
    '  }',
    '}',
    '',
    ...
    '  escala.forEach(dia => {',
    '    dia.turnos.forEach(turno => {',
    '      preenchidas += (dadosEscala[turno.id] || []).filter(n => n && n.trim()).length;',
    '    });',
    '  });',
    ...
    '  toast.className = "toast show " + classe;',
    ...
    '  const tf  = focado && focado.dataset && focado.dataset.turno;',
    '  const vf  = focado && focado.dataset && focado.dataset.vaga;',
    '  const val = focado && focado.value;',
